...
class irodsCreateTicket(QWidget, Ui_tabticketCreate):
    def __init__(self):
        ...
        self.irodsRootColl = f'/{CONN.zone}'
    ...
    def create_ticket(self):
        ...
        if not CONN.collection_exists(obj_path):
            self.infoLabel.setText('ERROR: Please select a collection.')
            self.setCursor(QtGui.QCursor(QtCore.Qt.CursorShape.ArrowCursor))
            self.createTicketButton.setEnabled(True)
            return
        acls = [(acl.user_name, acl.access_name) for acl in CONN.get_permissions(obj_path)]
        if (CONN.username, 'own') in acls:
            date = self.calendar.selectedDate()
            # format of time string for irods: 2012-05-07.23:00:00
            expiry_string = f'{date.toPyDate()}.23:59:59'
            ticket_name, expiration_set = CONN.create_ticket(obj_path, expiry_string)
            self.ticketInfoBrowser.append(f'iRODS server:\t{CONN.host}')
