...
For experimental feature tabs please refer to section @sec-features.
...
3. Lower tabs
   ...
   - Permissions: Will show the permissions own(er), read, write.
...
![](../screenshots/howto_upload.png)
 ...
 1. Check the resource. There may be several storage resources configured for you in the system. Your iRODS admin will give you advice which one to use for which data and workflow. The dropdown menu is preset with the `irods_default_resource` from the *.irods/irods_environment.json*.
...
- A preview of the data will be shown in teh fiel below the iRODS path.
...
::: {.callout-note collapse="true"}
## Experimental tabs and fetaures {#sec-experimental}
### Data bundeling
